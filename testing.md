...
Гораздо лучше воспользоваться возможностью python для форматирования строк. Дополнительно можно [почитать](https://realpython.com/python-string-formatting/#2-new-style-string-formatting-strformat)
...
```
Дважды считывать атрибут — это плохая практика, потому что при повторном считывании текст на странице может измениться, и вы получите неактуальный текст об ошибке. 
...
В этой конструкции вызвали функцию **test_abs1()**, которая выполняет тестовый сценарий.

С помощью **print("All tests passed!") вывели сообщение, если все тесты прошли успешно**.
